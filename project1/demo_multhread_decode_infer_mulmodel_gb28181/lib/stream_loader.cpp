...
void mpp_decoder_frame_callback(void *buffer, int width_stride, int height_stride, int width, int height, int format, int fd, void *data, int id)
{
    ...
    static thread_local RgaDma32Buffer bgr_dma32;
    const size_t bgr_bytes = (size_t)width * height * 3;
    bool rga_ok = false;

    if (bgr_dma32.ensure(bgr_bytes))
    {
        rga_buffer_t src_buf = wrapbuffer_virtualaddr_t(yuv_data, width, height, width, height, RK_FORMAT_YCbCr_420_SP);
        rga_buffer_t dst_buf = wrapbuffer_virtualaddr_t(bgr_dma32.data(), width, height, width, height, RK_FORMAT_BGR_888);
        IM_STATUS st = imcvtcolor(src_buf, dst_buf,
                                  RK_FORMAT_YCbCr_420_SP, RK_FORMAT_BGR_888,
                                  IM_COLOR_SPACE_DEFAULT);
        if (st == IM_STATUS_SUCCESS)
        {
            std::memcpy(mbuffer->bgr_work.data, bgr_dma32.data(), bgr_bytes);
            rga_ok = true;
        }
    }
